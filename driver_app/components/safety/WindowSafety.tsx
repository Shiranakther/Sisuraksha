...
const getAlertIcon = (alertType: string | null): keyof typeof Ionicons.glyphMap => {
  const type = (alertType || '').toLowerCase();
  if (type.includes('head')) return 'person';
  if (type.includes('hand')) return 'hand-left';
  if (type.includes('body')) return 'body';
  return 'alert-circle';
};

const getAlertTitle = (alertType: string | null): string => {
  if (!alertType) return 'Window Alert';
  return alertType.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase());
};

export default function WindowSafetyMonitor() {
  ...
  const fetchStatus = useCallback(async () => {
    try {
      ...
      if (response.data) setSystemStatus(response.data);
    } catch (error) {
  ...
  const fetchModelStatus = useCallback(async () => {
    try {
      ...
      if (response.data) setModelStatus(response.data);
    } catch (error) {
  ...
  const stats = useMemo(() => {
    const criticalCount = alerts.filter(a => a.status === 'DANGER' || a.status === 'CRITICAL').length;
    const warningCount = alerts.filter(a => a.status === 'WARNING').length;
    
    return {
      today: groupedAlerts.today.length,
      ...
      >
          ...
          <View className={`p-6 rounded-2xl ${modelStatus.running ? 'bg-indigo-500' : 'bg-slate-400'}`}>
            <View className="flex-row items-center justify-between">
              <View className="flex-row items-center">
                ...
                <View className="ml-4">
                  <Text className="text-white/70 text-sm uppercase tracking-wider">Window Area</Text>
        ...
        >
          <Ionicons 
            ...
            size={16} 
            color={autoRefresh ? "#6366F1" : "#94A3B8"} 
          />
          <Text className={`ml-1 text-sm ${autoRefresh ? 'text-indigo-500' : 'text-slate-400'}`}>
            Auto-refresh {autoRefresh ? 'on' : 'off'}
          </Text>
        </TouchableOpacity>

        <View className="mx-4 mt-4 bg-white rounded-2xl p-4 mb-6">
          <Text className="text-lg font-semibold text-slate-800 mb-4">Activity Logs</Text>
          
          {alerts.length === 0 ? (
            <View className="py-8 items-center">
              <Ionicons name="scan-circle-outline" size={40} color="#CBD5E1" />
                ...
                <TimelineGroup title="Today">
                  {groupedAlerts.today.map((alert, idx) => (
                    <AlertTimelineItem
                      ...
                      message={alert.message || 'Detection logged'}
                      severity={alert.status === 'DANGER' ? 'CRITICAL' : (alert.status || 'SAFE')}
