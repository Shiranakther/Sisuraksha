...
// + DFPlayer Mini alert when bus is moving and footboard is occupied
...
const int DFPLAYER_ALERT_TRACK = 1;  // SD root file: /0001.mp3
const int DFPLAYER_VOLUME = 25;      // 0 to 30
const unsigned long ALERT_REPEAT_MS = 5000;
...
const float SPEED_CALIBRATION = 0.25;  // Lower value = lower displayed speed.
...
unsigned long lastAlertPlayMs = 0;
...
void updateSpeedMeasurement() {
  ...
  if (pulses > 0) {
    ...
    currentSpeedKmh = rotationsPerSecond * WHEEL_CIRCUMFERENCE_M * 3.6 * SPEED_CALIBRATION;
    currentMoving = true;
...
void printDfPlayerStatus(uint8_t type, int value) {
  switch (type) {
    ...
    case 11:
      Serial.print("[DFPlayer] Track finished/status complete: ");
...
void initDfPlayer() {
  ...
  Serial.println("[DFPlayer] Trying normal mode...");
  dfPlayerReady = dfPlayer.begin(dfSerial);

  if (!dfPlayerReady) {
    Serial.println("[DFPlayer] Normal mode failed. Trying no-ACK mode...");
    dfPlayerReady = dfPlayer.begin(dfSerial, false, false);
  }

  if (!dfPlayerReady) {
    Serial.println("[DFPlayer] Not detected. Alert audio disabled.");
    return;
  }
  ...
  Serial.print("[DFPlayer] Alert file: /000");
  Serial.print(DFPLAYER_ALERT_TRACK);
  Serial.println(".mp3");
}

void updateDfPlayerAlert() {
  if (dfPlayerReady && dfPlayer.available()) {
    printDfPlayerStatus(dfPlayer.readType(), dfPlayer.read());
  }

  bool footboardOccupied = step1 || step2 || step3;
  bool shouldAlert = currentMoving && footboardOccupied;
  unsigned long nowMs = millis();

  if (!dfPlayerReady) {
    if (shouldAlert && !alertActive) {
      Serial.println("[ALERT] Bus moving + footboard occupied, but DFPlayer is not ready");
    }
    alertActive = shouldAlert;
    return;
  }

  if (shouldAlert && (!alertActive || nowMs - lastAlertPlayMs >= ALERT_REPEAT_MS)) {
    Serial.println("[ALERT] Bus moving + footboard occupied. Playing /0001.mp3");
    dfPlayer.play(DFPLAYER_ALERT_TRACK);
    lastAlertPlayMs = nowMs;
  }
...
void setup() {
  ...
  Serial.println(SPEED_CALIBRATION, 2);
  Serial.print("Min pulse gap us: ");
  Serial.println(MIN_PULSE_GAP_US);
  Serial.println("====================================");
  ...
  if (WiFi.status() != WL_CONNECTED) {
    Serial.println("\n[WiFi] Failed. Continuing DFPlayer safety test without dashboard.");
  } else {
    Serial.println();
    Serial.println("==============================");
    ...
    if (MDNS.begin("sisuraksha-ir-speed-dfplayer")) {
      Serial.println(" mDNS hostname: http://sisuraksha-ir-speed-dfplayer.local");
