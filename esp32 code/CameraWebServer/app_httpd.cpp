...
#include <lwip/sockets.h>  // for TCP_NODELAY socket optimization
...
#define STREAM_TARGET_FRAME_MS 90
#define STREAM_SEND_WAIT_TIMEOUT_SEC 2
#define STREAM_RECV_WAIT_TIMEOUT_SEC 2
...
static esp_err_t stream_handler(httpd_req_t *req) {
  ...
  // === Hardcore Optimization: Disable TCP Nagle's Algorithm ===
  // Nagle's algorithm delays IP packets to combine them, which brutally murders MJPEG video framing out over Wi-Fi.
  // By skipping the delay, we force the socket to push video frames INSTANTLY, 
  // eliminating network-layer shuttering and locking in a stable framerate stream.
  ...
  while (true) {
    ...
    if (res == ESP_OK) {
      size_t hlen = snprintf((char *)part_buf, 128, "\r\n--" PART_BOUNDARY "\r\nContent-Type: image/jpeg\r\nContent-Length: %u\r\nX-Timestamp: %d.%06d\r\n\r\n", _jpg_buf_len, (int)_timestamp.tv_sec, (int)_timestamp.tv_usec);
    ...
    if (frame_time < STREAM_TARGET_FRAME_MS) {
      vTaskDelay(pdMS_TO_TICKS(STREAM_TARGET_FRAME_MS - frame_time));
    } else {
      vTaskDelay(1);
    }
  }
...
// =============================================================
// Legacy handler kept unregistered. Direct WiFi credentials are configured
...
static esp_err_t reset_wifi_handler(httpd_req_t *req) {
  const char *html =
    ...
    "<title>Sisuraksha — WiFi Reset</title>"
    "<style>"
    "body{font-family:Arial,sans-serif;background:#0d0d1a;color:#eee;"
    "display:flex;align-items:center;justify-content:center;height:100vh;margin:0}"
    ".box{background:#16213e;border-radius:12px;padding:30px;text-align:center;max-width:340px}"
    "h2{color:#ff6b6b;margin-bottom:10px}p{color:#aaa;font-size:0.9em}"
    ".badge{background:#ff6b6b;color:#fff;border-radius:6px;padding:6px 16px;"
    "font-weight:bold;display:inline-block;margin-top:14px}"
    "</style></head><body>"
    "<div class='box'>"
    "<h2>&#x26A0; WiFi Credentials Cleared</h2>"
    "<p>The board is rebooting now.<br>Connect to the<br>"
    "<b>Sisuraksha-CAM-Setup</b><br>WiFi AP and open<br>"
    "<b>192.168.4.1</b> to configure a new network.</p>"
    "<div class='badge'>Rebooting...</div>"
    "</div></body></html>";

  httpd_resp_set_type(req, "text/html");
  httpd_resp_send(req, html, strlen(html));

  // Small delay so the browser receives the response before reboot
  vTaskDelay(pdMS_TO_TICKS(800));

  const char *msg = "Edit WIFI_SSID and WIFI_PASSWORD in CameraWebServer.ino, then reflash.";
...
void startCameraServer() {
  ...
  // Keep HTTP camera work away from the ESP32 WiFi task as much as possible.
  // This helps the stream recover instead of freezing when the network stalls.
  config.core_id = 1;
  config.stack_size = 8192;
  config.send_wait_timeout = STREAM_SEND_WAIT_TIMEOUT_SEC;
  config.recv_wait_timeout = STREAM_RECV_WAIT_TIMEOUT_SEC;
  ...
  stream_config.server_port += 1;
  stream_config.ctrl_port += 1;
  stream_config.max_uri_handlers = 2;
  log_i("Starting stream server on port: '%u'", stream_config.server_port);
