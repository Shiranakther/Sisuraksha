...
// E18-D80NK: LOW = object detected, HIGH = clear
#define DETECTED  LOW
...
// -- Python Footboard Webhook ------------------
// Set this to the laptop IP running footboard safety/riyabeth_pro_safety.py.
// Port 5001 is the Flask webhook; Node uses port 5000 for /api/safety routes.
...
void postToNode(bool s1, bool s2, bool s3) {
    ...
    if (code == 200 || code == 201) {
        Serial.printf("[Footboard] Posted OK | risk=%d\n", risk);
    } else {
        Serial.printf("[Footboard] POST failed: %d\n", code);
    }
...
void broadcastTask(void* param) {
    for (;;) {
        ...
        step1 = (digitalRead(IR_STEP1) == DETECTED);
        step2 = (digitalRead(IR_STEP2) == DETECTED);
        step3 = (digitalRead(IR_STEP3) == DETECTED);
...
void setup() {
    ...
    // IR sensor pins
    pinMode(IR_STEP1, INPUT);
    pinMode(IR_STEP2, INPUT);
    pinMode(IR_STEP3, INPUT);
